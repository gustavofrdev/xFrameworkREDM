...
nativeui_ilegal = Proxy.getInterface("nativeui_ilegal")
nativeui_ilegalSv = Tunnel.getInterface("nativeui_ilegal")
_Tunnel = {}
Tunnel.bindInterface("nativeui_ilegal", _Tunnel)
--------------------------------------------------------
local this_bando_craftPlacexyz = {x = 315.64, y = 1492.2, z = 181.38}
...
RegisterNUICallback('ButtonClick',function(data, cb)
        ...
        if data == 'fechar' then
            ...
            if string.find(data, "_comprar") then 
                data = string.gsub(data, "_comprar", "")
                nativeui_ilegalSv.comprar(data)
            else 
                nativeui_ilegalSv.vender(data)
            end
        end
    end
)
...
local local_venda = {
    {x = -1318.56, y = 2464.61, z = 310.05}, -- MUDAR A LOC AQUI
}
...
Citizen.CreateThread(function()
    for k,v in pairs(local_venda) do
        ...
        Citizen.InvokeNative(0x9CB1A1623062F402, blip[k], 'Venda de itens roubados')
    end
end)
