...
FrameworkSV = Tunnel.getInterface("x_API")
    ...
    if not IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
        ...
        while not HasAnimDictLoaded("script_common@jail_cell@unlock@key") do
            ...
            if waiting > 5000 then
                -- print("RedM Fucked up this animation, oh yea! Fuck my ass too!")
                break
            end
        end
        Wait(100)
        ...
        while key do
            if IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
                ...
                key = false
                __UpdateDoor(GetPlayerServerId(), k)
                break
            end
        end
    end
end
function __UpdateDoor(source, door )
    -- print(door)
    FrameworkSV.UpdateDoor(source, door) --Perms check
end

function _Tunnel.ChangeDoor(doorId )

    local name  = Config.DoorList[doorId]
    name.locked = not name.locked
    FrameworkSV.UpdateState(doorID, name.locked)
end
...
function _Tunnel.SetState(doorID, state)
