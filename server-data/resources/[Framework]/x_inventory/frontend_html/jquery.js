...
$(() => {
    ...
    // $('#close').click($.post('http://x_inventory/NUIFocusOff'))
    $('#use').droppable({
    ...
    $('#send').droppable({
        ...
        drop: function(event, ui) {
            ...
            //console.log('send')
            disableInventory(500)
            $.post('http://x_inventory/SendItem',
                JSON.stringify({
                    number: parseInt($('#count').val()),
                    data: itemData
                })
    ...
    document.onkeyup = data => {
        if (data.which == 27) {
            $.post('http://x_inventory/NUIFocusOff')
        }
    }
    ...
    window.addEventListener('message', event => {
        if (event.data.action === 'display') {
            ...
            $('#info-identity').html('<b>Nome:</b> '+event.data.name+' '+event.data.firstname+' ID: '+event.data.id+' <b>RG:</b> '+event.data.reg+' <b>Cel:</b> '+event.data.phone+' <br><b>Carteira:</b> $'+event.data.cash+' <b>Banco:</b> $'+event.data.bank+' <b>Multas:</b> $'+event.data.tax+' <br><b>Emprego:</b> '+event.data.job)
            ...
            if ( str.substring(0, str.indexOf("-")) != "drop" ){
                if (event.data.max != undefined && event.data.weight != undefined){
                    console.log(" here::: ) ")
    ...
    $('.slot').droppable({
        ...
        drop: function(event, ui) {
            ...
            if (secondary === 'drop') {
                ...
                disableInventory(500)
                $.post(
                    'http://x_inventory/TraficItem',
                    JSON.stringify({
                        number: parseInt($('#count').val()),
                        data: itemData,
                    })
                )
            }else if (secondary === "cartas"){
                disableInventory(500)
                $.post(
                    'http://x_inventory/TakeFromCartas',
                    JSON.stringify({
                        number: parseInt($('#count').val()),
                        data: itemData,
                    })
                ...
                disableInventory(500)
                $.post(
                    'http://x_inventory/TakeFromTrunk',
                    JSON.stringify({
                        number: parseInt($('#count').val()),
                        data: itemData,
                    })
                ...
                disableInventory(500)
                $.post(
                    'http://x_inventory/TakeFromChest',
                    JSON.stringify({
                        number: parseInt($('#count').val()),
                        data: itemData,
                    })
                ...
                disableInventory(500)
                $.post(
                    'http://x_inventory/TakeFromVault',
                    JSON.stringify({
                        number: parseInt($('#count').val()),
                        data: itemData,
                    })
