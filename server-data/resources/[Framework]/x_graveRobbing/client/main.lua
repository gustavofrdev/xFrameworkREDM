...
local minerblips = {
    {["CaveName"] = "Graveone", x = -236.82, y = 821.13, z = 124.22, ["HasRares"] = true},
}
            ...
            for i=1, #Rocks, 1 do
                ...
                if GetDistanceZTrue(pos,EntCoords) < 3 then
                    ...
                    if nearbyObject then
                        DrawText3D(EntCoords.x, EntCoords.y, EntCoords.z, 'Pressione ~e~[R]~p~ para roubar este túmulo')
                        
                        if whenKeyJustPressed("R") then
                            TriggerEvent("cancelando", true)
                            local W = math.random(8000,15000)
                            local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
                            local c=  math.random(100)
                            if c < 50 then 
                                x_graveRobbingSV.police(x, y, z)
                            end
                            MineAndAttach()
                            Wait(100)
                            ...
                            x_graveRobbingSV.Final(HasRareGems)
