$(document).ready(function() {
    ...
    $('.submit').on('click', function(e) {
        e.preventDefault();
        console.log("saveClothe();")
        $.post('http://clothing/saveClothes', GetOutput());
    });
...
document.onkeydown = (evt) => {
    ...
    if (isEscape) {
        $.post('http://clothing/closeWithoutBuy', JSON.stringify({}));
...
function keys(obj)
{
    ...
    function Load() {
        ...
        $('.arrow-right').on('click', function(e) {
            ...
            e.preventDefault();
            var m = $(this).parent().prev().attr('data-legend')
            m = m.replace("/", "")
            m = parseInt(m)
            console.log(m)
            if(parseFloat($(this).prev().val())+1 > m) return
            console.log(parseFloat($(this).prev().val())+1)
        ...
        $('.arrow-left').on('click', function(e) {
            ...
            e.preventDefault();
            if(parseFloat($(this).next().val())-1 < 1) return
        ...
        $('input').change(function() {
            $.post('http://clothing/updateClothes', GetOutput());
        });
    ...
    $('.exit').on('click', function(e) {
        e.preventDefault();
        $.post('http://clothing/exit');
    });

    // Rotate player
    $(document).keypress(function(e) {
        if (e.which == 113) { // Q pressed
            $.post('http://clothing/heading', JSON.stringify({
                value: -10
            }));
        }
        if (e.which == 120) { // x pressed
            $.post('http://clothing/defcam');
        }
		...
		if (e.which == 119) { // w pressed
            $.post('http://clothing/camera', JSON.stringify({
                offset: 0.05
            }));
        }
