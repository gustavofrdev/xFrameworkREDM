...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(interior_sets_for_valsaloon_interior) do
      if not IsInteriorEntitySetActive(valsaloon_interior_id,v) then
        ...
        --alentine Saloon Interior Activated")
      end
    end
end)
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(interior_sets_for_bra_interior) do
      if not IsInteriorEntitySetActive(bra_interior_id,v) then
        ...
        --("Braithwaite Interior Activated")
      end
    end
end)
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(interior_sets_for_rhoshrf_interior) do
      if not IsInteriorEntitySetActive(rhoshrf_interior_id,v) then
        ...
        --("Rhodes Sheriff Interior Activated")
      end
    end
end)
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(interior_sets_for_newcombank_interior) do
      if not IsInteriorEntitySetActive(newcombank_interior_id,v) then
        ...
        --("New Com Bank Interior Activated")
      end
    end
end)
...
AddEventHandler('DevDokus:PlayPiano:C:ActPiano', function(ped, x, y, z, h)
  ...
  if not IsPlaying then DrawInfo("Pressione Espaço para tocar o Piano", 0.5, 0.95, 0.75) end
    if IsPlaying then DrawInfo("F para parar", 0.5, 0.95, 0.75) end
      ...
      while true do Wait(1)
        ...
        for k, v in pairs(RiverHashes) do
          if water == v then
            if OnFoot then
              if InWater then
                local wash = CreateVarString(10, 'LITERAL_STRING', "Se lavar ou beber água")
                local drink = CreateVarString(10, 'LITERAL_STRING', "Se lavar ou beber água")
                ...
                if PromptHasHoldModeCompleted(Drinking) then
                  if IsPedMale(ped) then 
                    ...
                    exports["x_progress"]:startUI(17000, "Bebendo...")
                    TaskStartScenarioInPlace(ped, hash, -1, true, false, false, false)
                    Wait(17000) 
                    TriggerServerEvent("--> ++beber++ <--", 25)
                    ClearPedTasks(ped)
                    PlaySoundFrontend("Core_Fill_Up", "Consumption_Sounds", true, 0)
                  else 
                    -- FEMALE SCENARIO
                    ...
                    TaskStartScenarioInPlace(ped, GetHashKey('WORLD_PLAYER_DRINK_WITCHES_BREW'), 60000, true, false, false, false)
                    Wait(10000)
                    TriggerServerEvent("--> ++beber++ <--", 25)
                    ClearPedTasksImmediately(ped)
                    PlaySoundFrontend("Core_Fill_Up", "Consumption_Sounds", true, 0)
...
AddEventHandler("bandana", function()
	local ped = PlayerPedId()
...
function getKorrigan()
    local interior = GetInteriorAtCoords(3287.704, -1317.588, 42.761)
