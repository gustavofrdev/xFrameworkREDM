...
local function setMenuVisible(id, visible, holdCurrent, text)
	if id and menus[id] then
		...
		if not holdCurrent and menus[id] then
			...
			if text then
			setMenuProperty(id, 'script_id', text)
			end
		end
...
local function drawTitle()
	if menus[currentMenu] then
		...
		if HasStreamedTextureDictLoaded("menu_textures") and HasStreamedTextureDictLoaded("generic_textures") then
			...
			DrawSprite("menu_textures", "translate_bg_1a", x, y, menus[currentMenu].width + .03, titleHeight, 0.0, 200, 22, 30, 255, 0)
		end
...
local function drawSubTitle()
	if menus[currentMenu] then
		...
		drawText(menus[currentMenu].subTitle, menus[currentMenu].x + buttonTextXOffset + 0.03, y - buttonHeight / 2 + buttonTextYOffset, buttonFont, subTitleColor, buttonScale, false)
...
function WarMenu.ChangeMenuName(MenuID, menuNewTitle)
	setMenuProperty(MenuID, 'subTitle', string.upper(menuNewTitle))
end
...
function WarMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		...
		if subTitle then
			...
			setMenuProperty(id, 'currentSelect', id)
			-- setMenuProperty(id, 'currentSelectName', id)
		end
...
function WarMenu.MenuButton(text, id)
	if menus[id] then
		if WarMenu.Button(text) then
			setMenuVisible(currentMenu, false,text)
			setMenuVisible(id, true, true,text)
...
local k_delay3 = 160 -- et si touche restée appuyée.

function isPressedKey(key)
	if key ~= lastKey and key ~= keys.select and IsDisabledControlPressed(1, key) then -- Pas la même touche -> RESET
		lastKey = key
		timer = GetGameTimer()
		count = 0
		pass = false
	...
	elseif key == lastKey and IsDisabledControlPressed(1, key) then -- Meme Touche
		local a = false
		if a then 										-- Accélération du défilement
			count = 0
