...
barbershop = Proxy.getInterface('barbershop')
barbershopSv = Tunnel.getInterface('barbershop')
_Tunnel = {}
...
local currentblushIndex = 1
local currentblushcolorIndex = 1
local currentblushopacityIndex = 1 
local currentdelineadoIndex = 1
local currentdelinedoopacityIndex=  1
local currentdelineadocolorIndex = 1
local currentsombraIndex = 1
local currentsombracolorIndex = 1
local currentsombraopacityIndex = 1
local currentbatomIndex = 1
local currentbatomcolorIndex = 1
local currentbatomopacityIndex = 1
local ac_data = {
    cabelo = 1,
    barba  = 1,
} 
...
local active = false
local ShopPrompt
RegisterNetEvent("setAllBarberValues")
AddEventHandler("setAllBarberValues", function(blush_index, blush_color_index, blush_opacity_index, delineado_opacity_index, delineado_color_index,sombra_opacity_index, sombra_color_index, batom_opacity_index, batom_color_index, hair_index, beard_index)
    print(blush_opacity_index)
   currentblushIndex = blush_index
   currentblushcolorIndex = blush_color_index
   currentblushopacityIndex = blush_opacity_index
   currentdelinedoopacityIndex = delineado_opacity_index
   currentdelineadocolorIndex = delineado_color_index
   currentsombraopacityIndex = sombra_opacity_index
   currentsombracolorIndex = sombra_color_index
   currentbatomcolorIndex = batom_color_index
   currentbatomopacityIndex = batom_opacity_index
   ac_data.cabelo = hair_index
   ac_data.barba = beard_index
end)
local list_hair = {}
...
RegisterNetEvent("loadMakeup")
AddEventHandler("loadMakeup", function()
    if GetEntityModel(PlayerPedId()) ~= -171876066 and GetEntityModel(PlayerPedId()) ~= -1481695040 then return end
    if not IsPedMale(PlayerPedId()) then 
        TriggerEvent("__OVERLAYS__")
        Wait(500)
    if currentsombraopacityIndex and currentsombraopacityIndex > 1 then 
        print "shadows"
        toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(266),tonumber(255),tonumber(255), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
    end
    if currentblushIndex and currentblushIndex > 1 then 
        print "blush"
        toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(266),tonumber(266),tonumber(255),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(0),tonumber(0),tonumber(0),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(0),tonumber(0),tonumber(0),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
    end
    if currentbatomopacityIndex and currentbatomopacityIndex > 1 then
        print "lipsticks"
        print(currentbatomopacityIndex)
        toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(266),tonumber(266),tonumber(255), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
    end
    if currentdelinedoopacityIndex and currentdelinedoopacityIndex > 1 then 
        print "eyeliners"
        toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
        toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
        end
    end
...
function barberMenu()
    ...
    if sex == 1 then
        elements = {
            ...
            {label = "Hair", cat = "hair", value = ac_data.cabelo, type = 'slider', min = 1, max = list_hair_length, desc = "Choose your hairstyle"},
            {label = "Beard", cat = "mustache", value = ac_data.barba, type = 'slider', min = 1, max = list_mustache_length, desc = "Choose your beardstyle"},
        }
        currentHairList = list_hair
        currentBeardList = list_mustache
    else
        elements = {
                {label = "Confirmar", desc = "Pressione para comprar tudo!", cat = "confirm"},
                {label = "Hair", cat = "hair", value = ac_data.cabelo, type = 'slider', min = 1, max = list_hair_f_length, desc = "Choose your hairstyle"},
                {label = "Opcidade do Batom", cat = "batom_op", value = currentbatomopacityIndex, type = 'slider', min = 1, max = 100, desc = "Opacidade Batom"},
                {label = "Cor do Batom", cat = "batom_color", value = currentbatomcolorIndex, type = 'slider', min = 1, max = 26, desc = "Cor Batom"},
                {label = "Opcidade das Sombras", cat = "sombra_op", value = currentsombraopacityIndex, type = 'slider', min = 1, max = 100, desc = "Opacidade Sombras"},
                {label = "Cor das Sombras", cat = "sombra_color", value = currentsombracolorIndex, type = 'slider', min = 1, max = 26, desc = "Cor Sombras"},
                {label = "Opcidade do Delineado", cat = "delineado_op", value = currentdelinedoopacityIndex, type = 'slider', min = 1, max = 100, desc = "Opacidade Delineado"},
                {label = "Cor do Delineando", cat = "delineado_color", value = currentdelineadocolorIndex, type = 'slider', min = 1, max = 26, desc = "Cor Delineado"},
                {label = "Blush", cat = "blush", value = currentblushIndex, type = 'slider', min = 1, max = #overlays_info["blush"] , desc = "blushs"},
                {label = "Opcidade do Blush", cat = "blush_op", value = currentblushopacityIndex, type = 'slider', min = 1, max = 100, desc = "Opacidade blush"},
                {label = "Cor do Blush", cat = "blush_color", value = currentblushcolorIndex, type = 'slider', min = 1, max = 26, desc = "Cor blush"},
            }
    ...
    function(data, menu)
        ...
        if data.current.cat == "confirm" then
            ...
            print("blush_index_op :::::::", currentblushopacityIndex)
            barbershopSv.buy(currentblushIndex, currentblushcolorIndex, currentblushopacityIndex, currentdelinedoopacityIndex, currentdelineadocolorIndex,currentsombraopacityIndex, currentsombracolorIndex, currentbatomopacityIndex, currentbatomcolorIndex, ac_data.cabelo, ac_data.barba)
        end
    end,
	function(data, menu)
		-->>("closed Menu")
	...
	function(data, menu)
        ...
        if data.current.cat == "hair" then
            if data.current.value <= 1 then return end;
            if data.current.value > 1 then 
                ac_data.cabelo = data.current.value
        ...
        elseif data.current.cat == "sombra" then 
            if data.current.value <= 1 then currentsombraIndex = 1 return end;
            currentsombraIndex = data.current.value
            toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(266),tonumber(255),tonumber(255), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "sombra_op" then 
            if data.current.value <= 1 then currentsombraopacityIndex = 1 return end;
            currentsombraopacityIndex = data.current.value
            toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "sombra_color" then 
            if data.current.value <= 1 then currentsombracolorIndex = 1 return end;
            currentsombracolorIndex = data.current.value
            toggleOverlayChange("shadows",1,tonumber(1),0,0,0,1.0,0,tonumber(currentsombracolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentsombraIndex), tonumber(currentsombraopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "blush" then 
            if data.current.value <= 1 then currentblushIndex = 1 return end;
            currentblushIndex = data.current.value
            toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(266),tonumber(266),tonumber(255),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "blush_op" then
            if data.current.value <= 1 then currentblushopacityIndex = 1 return end;
            if data.current.value > 1  then
              currentblushopacityIndex = data.current.value
              toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(0),tonumber(0),tonumber(0),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
              
             end
        elseif data.current.cat == "blush_color" then 
            if data.current.value <= 1 then currentblushcolorIndex = 1 return end;
            currentblushcolorIndex = data.current.value
            toggleOverlayChange("blush",1,tonumber(currentblushIndex),0,0,0,1.0,0,tonumber(currentblushcolorIndex),tonumber(0),tonumber(0),tonumber(0),tonumber(0), tonumber(currentblushopacityIndex/100) , PlayerPedId())
            
            -->>(data.current.cat)
        elseif data.current.cat == "batom" then 
            -->>('bat  __ :: <1>')
            currentbatomIndex = data.current.value
            toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(266),tonumber(266),tonumber(255), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "batom_op" then 
            if data.current.value <= 1 then currentbatomopacityIndex = 1 return end;
            -->>('bat  __ :: <2>')
            currentbatomopacityIndex = data.current.value
            toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "batom_color" then 
            if data.current.value <= 1 then currentbatomcolorIndex = 1 return end;
            -->>('bat  __ :: <2>')
            currentbatomcolorIndex = data.current.value
            toggleOverlayChange("lipsticks",1,tonumber(1),0,0,0,1.0,0,tonumber(currentbatomcolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(batomcurrentbatomIndex), tonumber(currentbatomopacityIndex/100) , PlayerPedId())
            
            --> Delineado
            -->>(data.current.cat)
        elseif data.current.cat == "delineado" then 
            if data.current.value <= 1 then currentdelineadoIndex = 1 return end;
            -->>('bat  __ :: <1>')
            currentdelineadoIndex = data.current.value
            toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
        elseif data.current.cat == "delineado_op" then 
            if data.current.value <= 1 then return end;
            -->>('bat  __ :: <2>')
            currentdelinedoopacityIndex = data.current.value
            toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
            
        elseif data.current.cat == "delineado_color" then 
            if data.current.value <= 1 then currentdelineadocolorIndex = 1 return end;
            -->>('bat  __ :: <2>')
            currentdelineadocolorIndex = data.current.value
            toggleOverlayChange("eyeliners",1,tonumber(1),0,0,0,1.0,0,tonumber(currentdelineadocolorIndex),tonumber(0),tonumber(0),tonumber(0), tonumber(currentdelineadoIndex), tonumber(currentdelinedoopacityIndex/100) , PlayerPedId())
            
        if data.current.cat == "mustache" then 
            if data.current.value > 1 then 
                ac_data.barba = data.current.value
...
Citizen.CreateThread(function()
	while true do
        ...
        for k, v in pairs(Config.Locais) do 
            if GetDistanceBetweenCoords(v.blip.x, v.blip.y, v.blip.z, GetEntityCoords(PlayerPedId())) < 4 then
                W = 1
                if not active then 
                    DrawText3D( v.blip.x, v.blip.y, v.blip.z-1, "Pressione ~e~[R]~p~ para acessar a barbearia!")
                end
                if IsControlJustPressed(0, 0xE30CD707) then 
                    ...
                    active = true 
                    print(v.blip.x, v.blip.y, v.blip.z)
                    print(v.cadeira.x, v.cadeira.y, v.cadeira.z, v.cadeira.w)
                    TaskStartScenarioAtPosition(GetPlayerPed(), GetHashKey("PROP_PLAYER_BARBER_SEAT"), v.cadeira.x, v.cadeira.y, v.cadeira.z, v.cadeira.w, 100000, true, 0)
                    Wait(8000)
                    barberMenu()
                    ::continue::
                end
