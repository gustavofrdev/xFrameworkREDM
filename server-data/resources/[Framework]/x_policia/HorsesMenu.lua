            ...
            for k, v in pairs(config.spawnPositions) do 
                if GetDistanceBetweenCoords(playerCoords, v["BlipPos"][1],v["BlipPos"][2],v["BlipPos"][3], true) < 4.0 then 
                   ...
                   if GetDistanceBetweenCoords(playerCoords, v["BlipPos"][1],v["BlipPos"][2],v["BlipPos"][3], true) < 1.0 then 
                    DrawText3D( v["BlipPos"][1],v["BlipPos"][2],v["BlipPos"][3]-1, "Pressione ~e~[G]~p~ para acessar a lista de cavalos policiais")
                        if IsControlJustPressed(0, 0x760A9C6F) then 
                            if x_policiaSv.permissao() then 
                                WarMenu.OpenMenu("Cavalos-Policiais")
...
Citizen.CreateThread(function()
WarMenu.CreateMenu("Cavalos-Policiais", "Cavalos Policiais")
while true do 
        if WarMenu.IsMenuOpened("Cavalos-Policiais") then 
            for k, v in pairs(config.policeHorses) do
                if WarMenu.Button(v) then
                    loadnpc(k)
                    print(x, y, z, k )
                    spawnNewPoliceHorse(k, {x = x, y = y, z = z})
                end
            end
